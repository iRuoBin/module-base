...
class ScreenShotListenManager {
    ...
    /** 读取媒体数据库时需要读取的列, 其中 WIDTH 和 HEIGHT 字段在 API 29 以后才有  */
    private val MEDIA_PROJECTIONS_API_Q = arrayOf(
    ...
    /** 已回调过的路径  */
    private val sHasCallbackPaths = arrayListOf<String>()
    ...
    private constructor(context: Context?) {
        ...
        if (sScreenRealSize == null) {
            ...
            if (sScreenRealSize != null) {
                Log.d(
                    TAG,
                    "Screen Real Size: " + sScreenRealSize?.x.toString() + " * " + sScreenRealSize?.y
                )
            } else {
    ...
    private fun handleMediaContentChange(contentUri: Uri) {
        ...
        try {
            ...
            cursor = mContext!!.contentResolver.query(
                contentUri,
                if (Build.VERSION.SDK_INT < 16) MEDIA_PROJECTIONS
                else if (Build.VERSION.SDK_INT < Build.VERSION_CODES.Q) MEDIA_PROJECTIONS_API_16
                else
                    MEDIA_PROJECTIONS_API_Q,
                null,
                null,
            ...
            // 获取各列的索引
            val dataIndex: Int = cursor.getColumnIndex(MediaStore.Images.ImageColumns.DATA)
            ...
            val dateTaken: Long = cursor.getLong(dateTakenIndex)
            var width = 0
            var height = 0
     ...
     * 媒体内容观察者(观察媒体数据库的改变)
     */
    inner class MediaContentObserver(private val contentUri: Uri, handler: Handler?) : ContentObserver(handler) {
